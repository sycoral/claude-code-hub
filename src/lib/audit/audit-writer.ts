...
export async function writeAuditRecord(
  ...
  try {
    ...
    await db
      ...
      .onConflictDoUpdate({
        target: conversationAuditLog.sessionId,
        set: {
          inputTokens: sql`COALESCE(${conversationAuditLog.inputTokens}, 0) + ${usage?.inputTokens ?? 0}`,
          outputTokens: sql`COALESCE(${conversationAuditLog.outputTokens}, 0) + ${usage?.outputTokens ?? 0}`,
          costUsd: sql`COALESCE(${conversationAuditLog.costUsd}, 0) + ${Number(usage?.costUsd ?? 0)}`,
          requestCount: sql`${conversationAuditLog.requestCount} + 1`,
          totalMessages: allMessages.length,
          contentSummary: summary || undefined,
          contentSize: sql`COALESCE(${conversationAuditLog.contentSize}, 0) + ${lineSize}`,
          updatedAt: new Date(),
        },
      });

    logger.debug({ sessionId, contentPath }, "Audit record upserted");
  } catch (err) {
...
function reassembleSSEResponse(sseText: string): unknown {
  ...
  for (const evt of events) {
    ...
    if (data.type === "response.completed" && typeof data.response === "object" && data.response) {
      ...
      if (typeof resp.usage === "object" && resp.usage)
